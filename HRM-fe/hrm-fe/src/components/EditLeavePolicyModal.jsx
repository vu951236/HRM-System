...
const EditLeavePolicyModal = ({ policy, onClose, onUpdate }) => {
    ...
    useEffect(() => {
        if (policy) {
            const matchedKey = Object.keys(rolePositionMap).find(
                key =>
                    rolePositionMap[key].roleName === policy.roleName &&
                    rolePositionMap[key].positionName === policy.positionName
            );
    ...
    return (
        <div className="modal-overlay">
            <div className="modal">
                <h2>✏️ Chỉnh sửa chính sách nghỉ</h2>
                ...
                <div style={{ marginTop: '20px', textAlign: 'right' }}>
                    ...
                    <button onClick={handleSubmit}>Cập nhật chính sách</button>
                </div>
            </div>
        </div>
    );
